<template>
    <div class="meg-gridrows">
        <Gird-Row v-for="(row, index) in rows" v-show="isShow(index)" :row="row" :startColumnIndex="startColumnIndex"
            :endColumnIndex="endColumnIndex" :key="row.rowIndex" :style="getStyle(index)" />
    </div>
</template>
