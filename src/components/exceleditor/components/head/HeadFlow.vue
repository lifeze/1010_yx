<template>
    <div :class="className">
        ...
        <div v-for="(cell, index) in getCells()" :key="index" class="meg-head-index" :style="getCellStyles(cell)"
            @mousemove="isCanResize($event, cell)" @mousedown="handlerMousedown($event, cell)"
...
export default {
    ...
    methods: {
        ...
        getCellStyles(index) {
            const style = {};
            if (this.direction === 'column') {
                style.width = `${this.getCellSize(index)}px`;
            } else {
                style.height = `${this.getCellSize(index)}px`;
            }
