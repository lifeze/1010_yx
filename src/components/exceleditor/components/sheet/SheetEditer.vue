<template>
    <div class="meg-sheetediter meg-sheetediting" contenteditable="true" spellcheck="false" autocomplete="false"
        role="textbox" autocorrect="off" :style="styles" v-if="editState" @keydown.enter.stop="enter"
        @keydown.esc.stop="cancelEdit" @keyup.stop @keydown.stop @input="input" @paste="paste" @blur="blur" v-html="val"
        v-focus />
</template>
<script>
    ...
    computed: {
        ...
        styles() {
            ...
            return Object.assign(
                {
                    'min-width': `${info.width - 6}px`,
                    'min-height': `${info.height - 4}px`,
                    top: `${info.top - this.$sheet.scrollTop + 1}px`,
                    left: `${info.left - this.$sheet.scrollLeft + 1}px`
                },
                this.cellStyle
            );
        },
